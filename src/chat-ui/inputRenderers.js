 ...
 * by creating the corresponding DOM elements and attaching event listeners to handle user input.
 */
		...
		if (field.options && Array.isArray(field.options)) {
		  field.options.forEach((opt) => {
			...
			btn.addEventListener("click", () => {
			  if (field.multiple) {
				// In multiple selection mode, toggle the "active" class.
				btn.classList.toggle("active");
				// Gather all active button values.
				const activeButtons = Array.from(control.querySelectorAll("button.active"));
				const values = activeButtons.map(b => b.getAttribute("data-value"));
				updateSubmitButtonState(field, values);
			  } else {
				// In single selection mode, remove "active" from all buttons.
